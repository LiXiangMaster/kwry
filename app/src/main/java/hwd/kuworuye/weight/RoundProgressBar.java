...
public class RoundProgressBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int percent = (int) (((float) progress / (float) max) * 100);  //中间的进度百
//		分比，先转换成float在进行除法运算，不然都为0
        float textWidth = paint.measureText(percent + "%");   //测量字体宽度，我们
//		需要根据字体的宽度设置在圆环中间
//      && percent != 0 设置这个可以是否当进度为0时显示在 控件的中间；
        ...
        RectF oval = new RectF(centre - radius, centre - radius, centre
                + radius, centre + radius);  //用于定义的圆弧的形状和大小的 界限


        switch (style) {
            case STROKE: {
                paint.setStyle(Paint.Style.STROKE);
                canvas.drawArc(oval, 0, 360 * progress / max, false, paint);  //根 据进度画圆弧
